...
class RobustnessEvaluationTest(unittest.TestCase):
    ...
    def test_boundary_fix_compares_original_and_fixed_views(self):
        ...
        args = types.SimpleNamespace(
            ...
            data="custom",
            data_path="synthetic.csv",
            dataset_name="SyntheticBoundary",
            seq_len=4,
            cycle=24,
            random_seed=2024,
        )
        ...
        original_batch_x = batch_x.clone()
        # RecordingGTR returns the last two input values. The fixed view is
        # therefore exactly equal to this target, while the original is not.
        batch_y = torch.tensor([[[3.0], [3.0]], [[6.0], [6.0]]])
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            try:
                os.chdir(temp_dir)
                result = experiment.test("synthetic_boundary_setting")
                result_file = (
                    Path("results")
                    / "synthetic_boundary_setting"
        ...
        torch.testing.assert_close(batch_x, original_batch_x)
        self.assertEqual(result["dataset"], "SyntheticBoundary")
        self.assertEqual(
            result["boundary_fix"],
            {
                "enabled": True,
        ...
        self.assertEqual(result["fixed"]["mse"], 0.0)
        self.assertGreater(result["improvement_percent"]["mse"], 0.0)
